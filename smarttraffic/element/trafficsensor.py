...
class TrafficSensorTask(task_manager.Task):
    ...
    def __init__(self, sensor: TrafficSensor):
        super().__init__(f'ts/{sensor.slug}/main', 0.0, True)
        self.sensor = sensor
    ...
    def execute(self):
        if self.sensor.state == TrafficSensorState.NONE:
            ...
            velocity = self.sensor.calc_velocity(last_state_duration)

            cprint(f'[TRAFFICSENSOR/{self.sensor.slug}] Traffic - Velocity: {velocity} - Time: {last_state_duration}', 'cyan')
